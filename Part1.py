...
def encodeAC(msg, freq):
    ...
    for i in range(msglen):
        ...
        while stradle:
            # print("T")
            if low >= 0.5 and high < 1.0:
                bincode += "1"
                # print("1")
                low = 2 * (low - 0.5)
                high = 2 * (high - 0.5)
            elif low > 0 and high < 0.5:
                bincode += "0"
                # print("0")
                low = 2 * low
                high = 2 * high
                ...
                # print("stadle")
                stradle = False
    # print(bincode, low, high)
    bincode += "1"
    freqlist = []
    for k, v in freq.items():
        freqlist.append([k, v])
    # print("E", bincode)
    return bincode, (len(bincode) - 1), msglen, freqlist


def check(tag, low, high, freq):
    diff = high - low
    for i in range(len(freq)):
        per = freq[i][1][1] - freq[i][1][0]
        val = low + diff * per
        high = val
        # print("sada", low, high, tag)
        if tag < high and tag > low:
            # print("t")
            return freq[i][0], low, high
        low = val
    return freq[i][0], low, high


def decodeAC(bincode, codelen, msglen, freq):
    t = bin2float(bincode[0:codelen])
    # print(t)
    low = 0.0
    high = 1.0
    string, low, high = check(t, low, high, freq)
    # print(string, low, high)
    while len(string) != (msglen):
        if low >= 0.5 and high < 1.0:
            low = 2 * (low - 0.5)
            high = 2 * (high - 0.5)
            bincode = bincode[1:] + "0"
            t = bin2float(bincode)
            # print("111", t, low, high)
        elif low > 0 and high < 0.5:
            low = 2 * (low)
            high = 2 * (high)
            bincode = bincode[1:] + "0"
            ...
            # print(t, low, high)
            y, low, high = check(t, low, high, freq)
            string += y
            # print("stadle", string)
    # print(string)
    return string
...
for i in range(len(chunkdata) - 1):
    a, b, c, d = encodeAC(chunkdata[i], ftable)
    encodedata.append([a, b, c, d])
...
for i in range(0, 5):
    ...
    # print("val", diff_letters(binstr, x_str2))
    step = 0
    output = ""
    for j in encodedata:
        binstrchunk = x_str2[step : step + j[1] + 1]
        deco = decodeAC(binstrchunk, j[1], j[2], j[3])
        output += deco
        step = step + j[1] + 1
